    ...
    <div style={{ paddingBottom: 100 }}>
      ...
      <div style={{ padding: '0 16px', display: 'flex', flexDirection: 'column', gap: 16, marginTop: -4 }}>
        ...
        <div style={{
          ...
          <div style={{ position: 'absolute', bottom: -20, left: -20, width: 80, height: 80, borderRadius: '50%', background: 'rgba(77,163,255,0.15)', pointerEvents: 'none' }} />

          <ProgressRing score={score} size={110} strokeWidth={7} />

          <div style={{ flex: 1, zIndex: 1 }}>
            <p style={{ fontSize: 10, fontWeight: 600, color: 'rgba(255,255,255,0.55)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 5 }}>
              Recovery Status
            </p>
        ...
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10 }}>
            {[
              {
                label: 'Pain', icon: <Zap size={16} />,
                value: pain != null ? `${pain}/10` : '–',
                color: pain > 6 ? 'var(--color-danger)' : pain > 3 ? 'var(--color-warning)' : 'var(--color-lavender)',
              },
              {
                label: 'Bleeding', icon: <Droplet size={16} />,
                value: bleeding,
                color: bleeding === 'Yes' ? 'var(--color-danger)' : 'var(--color-success)',
              },
              {
                label: 'Stool', icon: <Activity size={16} />,
          ...
          <div style={{
            ...
            <ResponsiveContainer width="100%" height={56}>
              <LineChart data={weekData}>
                <Line type="monotone" dataKey="pain" stroke="var(--color-indigo)" strokeWidth={2.5}
                  dot={{ fill: 'var(--color-indigo)', r: 3 }} connectNulls />
                <Tooltip
                  contentStyle={{
                    background: 'var(--color-surface)', border: '1px solid var(--color-border)',
                    borderRadius: 8, fontSize: 12, fontFamily: 'var(--font-main)',
                  }}
                  formatter={v => [v, 'Pain']}
                />
              </LineChart>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 6 }}>
              {weekData.map((d, i) => (
                <span key={i} style={{ fontSize: 10, color: 'var(--color-text-muted)', flex: 1, textAlign: 'center' }}>{d.day}</span>
              ))}
            </div>
          </div>
        )}
        ...
        <button onClick={() => onNavigate('log')} style={{
          ...
          fontFamily: 'var(--font-main)', transition: 'opacity 0.18s ease',
        }}>
          <PlusCircle size={20} />
          Log today
        </button>
